...
, isnull([Off],0) as [Off]
from
(select a.stop_refid as [ID], c.[stop_name] as [Short], a.stop_sequence as [Order], passType_refid as [Rider], sum(b.passenger_on_count) as [Cnt]
...
{{{tripFilter}}}
{{{stopFilter}}}
group by a.stop_refid, c.[stop_name], a.stop_sequence, b.passType_refid
) as magicTable
pivot
(SUM([Cnt]) for [Rider] in ([CHILD],[DIS],[ELD],[EMP],[PCA],[RF],[STP],[TCA],[VET],[On])) as pvt
full join (select a.stop_refid, a.stop_sequence, sum(a.[PassengersOff]) as [Off] from [STRoute].[dbo].[passengers_bystop] a
where a.stop_refid is not null
{{{dateRangeFilter}}}
{{{dayFilter}}}
{{{routeFilter}}}
...
{{{tripFilter}}}
{{{stopFilter}}}
group by a.stop_refid, a.stop_sequence) as a
on [ID] = a.stop_refid
and [Order] = a.stop_sequence
order by [Order]
