...
, isnull([Off],0) as [Off]
from 
(select route_refid + '  ('+isnull(substring(cast(d.trip_startdeparttime as varchar),1,5), '')+ ')' as [Name]
...
join (select trip_refid, trip_startdeparttime from [STRoute].[dbo].[trips]) d
...
{{{tripFilter}}}
{{{stopFilter}}}
group by route_refid + '  ('+isnull(substring(cast(d.trip_startdeparttime as varchar),1,5), '')+ ')', passType_refid
) as magicTable
pivot
(SUM([Cnt]) for [Rider] in ([CHILD],[DIS],[ELD],[EMP],[PCA],[RF],[STP],[TCA],[VET],[On])) as pvt
left join (select route_refid + '  ('+isnull(substring(cast(d.trip_startdeparttime as varchar),1,5), '')+ ')' as [Name]
, sum([PassengersOff]) as [Off]
from [STRoute].[dbo].[passengers_bystop] a
left join (select trip_refid, trip_startdeparttime from [STRoute].[dbo].[trips]) d
on a.trip_refid = d.trip_refid
...
{{{tripFilter}}}
{{{stopFilter}}}
group by route_refid + '  ('+isnull(substring(cast(d.trip_startdeparttime as varchar),1,5), '')+ ')') as a
on pvt.[Name] = a.[Name]
order by pvt.[name]
